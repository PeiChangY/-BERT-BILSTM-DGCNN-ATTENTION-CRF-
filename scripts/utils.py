...
from scripts.estimate import Precision, Recall, F1_score,Precision_eve,Recall_eve,F1_score_eve
...
def train(model,
    ...
    tqdm_batch_iterator = tqdm(dataloader)  #Tqdm 是一个快速，可扩展的Python进度条，可以在 Python 长循环中添加一个进度提示信息
...
def valid(model,
          dataloader):
    model.eval() #预测模式 batchNorm层，dropout层等用于优化训练而添加的网络层会被关闭，从而使得评估时不会发生偏移。
    device = model.device
    pre_output = []
    true_output = []
    epoch_start = time.time()
    ...
    with torch.no_grad():  #关闭梯度计算
        ...
        for _, batch in enumerate(tqdm_batch_iterator):
            ...
            feats = model(inputs, masks)
            loss = model.loss(feats, tags, masks)
            out_path = model.predict(feats, masks) #将数据放到训练好的模型中预测
            pre_output.append(out_path) #append()添加对象到末尾
    ...
    precision_1,precision = Precision_eve(pre_output, true_output)
    recall_1,recall = Recall_eve(pre_output, true_output)
    f1_score_1 = F1_score_eve(precision_1, recall_1)
    estimator_1 = (precision_1, recall_1, f1_score_1)

    f1_score = F1_score(precision, recall)
    estimator = (precision, recall, f1_score)
    return epoch_time, epoch_loss, estimator,estimator_1
